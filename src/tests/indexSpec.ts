...
describe('Test responses from endpoints', (): void => {
  ...
  describe('endpoint: /api/images', (): void => {
    it('gets /api/images?filename=one (valid args)', async (): Promise<void> => {
      const response: supertest.Response = await request.get(
        '/api/images?filename=one'
      );

      expect(response.status).toBe(200);
    });

    it('gets /api/images?filename=one&width=199&height=199 (valid args)', async (): Promise<void> => {
      const response: supertest.Response = await request.get(
        '/api/images?filename=one&width=199&height=199'
      );

      expect(response.status).toBe(200);
    });

    it('gets /api/images?filename=one&width=-200&height=200 (invalid args)', async (): Promise<void> => {
      const response: supertest.Response = await request.get(
        '/api/images?filename=one&width=-200&height=200'
      );
